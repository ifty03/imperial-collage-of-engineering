...
const DepartmentList = () => {
    ...
    const { deleteData } = useDeleteData(`${server_url}/department/deleteDepartment`);
        ...
        <div className="">
            <AdminDashboardBreadcrumb
                items={[
                    ...
                    { name: "Add Department", path: "department/department-list" },
                ]}
            />
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 my-6">
                ...
                <Link to="/admin-dashboard/department/add-department" className="bg-green-600 hover:bg-green-700 text-white font-semibold rounded-[8px] px-10 py-2 text-lg transition">
                    Add Department
                </Link>
            </div>
            {/* Table */}
                ...
                <table className="min-w-[900px] w-full">
                    <thead className="bg-[#EAF2FA]">
                        <tr className=" text-gray-500 text-left text-sm">
                            ...
                            <th className="py-4 px-4 font-medium">Thumbnail</th>
                            <th className="py-4 px-4 font-medium">Name</th>
                            <th className="py-4 px-4 font-medium">Total Credit</th>
                            <th className="py-4 px-4 font-medium">Available Seat</th>
                    ...
                    <tbody>
                        {data?.data?.map((department, idx) => (
                            <tr key={department?._id} className="border-b last:border-b-0">
                                ...
                                <td className="py-4 px-4 font-medium text-gray-900">{department?.name}</td>
                                <td className="py-4 px-4 text-gray-700">{department?.totalCredit}</td>
                                <td className="py-4 px-4 text-gray-700">{department?.seatAvailable}</td>
                                <td className="py-4 px-4">
                                    <span
                                        className={`px-3 py-1 rounded-full text-sm font-semibold ${department?.status === "Active"
                                ...
                                <td className="py-5 px-4 flex gap-4 items-center">
                                    <Link to={`/admin-dashboard/department/edit/${department?._id}`} className="flex font-semibold items-center gap-1 text-blue-600 text-base">
